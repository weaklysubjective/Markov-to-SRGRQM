#!/usr/bin/env python3
"""
PP_markov_tau_geometry_v4_multiShellIntersect.py

STRICT PP Markov τ-geometry (v4) — 512x512-safe, sparse + matrix-free.

Design matches v3 intent:
- Loads flat + curved CA graph edges.
- Loads trace weights (experiences only; combined experiences allowed).
- Defines T00(i) = normalized trace weights.
- Defines MASS CORE strictly from traces via:
...
Key v4 changes (STRICT PP compliant):
- NO dense NxN P allocation.
- Build scipy sparse adjacency directly from edgelists.
- Build torch.sparse row-stochastic P.
- Compute hitting times with matrix-free fixed-point iteration:
      h_i = 0 for i in S
      h_i = 1 + Σ_j P_ij h_j  otherwise
  This avoids forming (I - P_oo) or any dense blocks.
...
def get_device(dev: str) -> torch.device:
    ...
    dev = dev.lower()
    if dev == "auto":
        # ROCm still reports via torch.cuda.*
...
def load_trace_weights(path: str, expected_N: int) -> np.ndarray:
    """
    Loads a trace/experience weights file.

    Supports:
    - one float per line
    - "idx weight"
    - multiple columns per line -> we take the LAST column
    - if the file is whitespace matrix with exactly expected_N rows and >1 cols,
      we allow summing across cols via --trace_mode sumcols (see CLI).
    """
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            if not s:
                continue
            parts = s.split()
            # allow "idx weight" or just "weight" or multi-col
            try:
                if len(parts) == 1:
...
def load_edges_as_adjacency_list(path: str, N: int) -> List[List[int]]:
    """
    Undirected adjacency list for BFS hop distances.
    Duplicates are tolerated to keep build fast at 512^2.
    """
    if not os.path.exists(path):
...
def load_edges_as_sparse_adj(path: str, N: int, symmetrize: bool = False):
    """
    Build a scipy sparse adjacency matrix from edgelist.

    Edge format: "src dst" or "src dst weight".
    We treat missing weight as 1.0.

    If symmetrize=True, we enforce undirected adjacency:
        A = 0.5*(A + A.T)
    """
    ...
    if len(rows) == 0:
        raise ValueError(f"No valid edges parsed from {path}")

    A = sp.coo_matrix((np.asarray(vals, dtype=np.float64),
                       (np.asarray(rows, dtype=np.int64),
                        np.asarray(cols, dtype=np.int64))),
                      shape=(N, N), dtype=np.float64).tocsr()

    if symmetrize:
        # For τ-geometry hitting-times we want a well-behaved stochastic walk.
        # This does NOT inject geometry; it only enforces an undirected edge-set
        # when the edgelist represents adjacency rather than directed causal flow.
        A = 0.5 * (A + A.T)
        A.eliminate_zeros()
...
def build_P_from_sparse_adj(A, device="cpu"):
    """
    Build row-stochastic Markov operator P from scipy sparse adjacency A.
    ...
    P = torch.sparse_coo_tensor(idx, v, size=(N, N),
                                dtype=torch.float64, device=device).coalesce()
    return P


def apply_P(P: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    if x.ndim == 1:
...
# ----------------------------
# Matrix-free hitting time solver
# ----------------------------

def mean_hitting_time_to_set_iter(
    P: torch.Tensor,
    ...
    """
    Matrix-free solver for mean hitting times h(i -> S).
    ...
    This avoids forming (I - P_oo) and is the only practical
    STRICT-PP way to scale the v3 construction to 512x512.

    Returns numpy float64 array length N.
    """
    N = P.shape[0]
    ...
    S_t = torch.from_numpy(S).to(device=device)
    ...
    for it in range(1, max_iter + 1):
        ...
        h_new = ones + Ph
        # enforce boundary
        h_new = torch.where(S_t, torch.zeros_like(h_new), h_new)

        if it % check_every == 0 or it == 1:
            diff = (h_new - h).abs()
            err = float(diff.max().detach().cpu().item())
            last_err = err
    ...
    # If not converged, still return best effort with a warning-like NaN guard
    if last_err is not None and (not math.isfinite(last_err) or last_err >= tol):
        # We keep values; no hard fail to preserve batch robustness.
        pass
...
def main():
    parser = argparse.ArgumentParser(
        description="PP Markov τ-geometry v4 (multi-shell intersect). STRICT PP, 512-safe."
    )
    ...
    # Hitting-time iteration controls (v4)
    parser.add_argument("--ht_tol", type=float, default=1e-6)
    parser.add_argument("--ht_maxiter", type=int, default=20000)
    parser.add_argument("--ht_check_every", type=int, default=50)

    # Adjacency handling
    parser.add_argument(
        "--symmetrize_P",
        action="store_true",
        help="If set, symmetrize adjacency before building P (A=0.5*(A+A.T))."
    )
    ...
    # Load graphs for BFS hop distances
    adj_flat_list = load_edges_as_adjacency_list(args.edges_flat, N=N)
    adj_curved_list = load_edges_as_adjacency_list(args.edges_curved, N=N)

    # Distances from mass core on both graphs
    d_flat = multisource_bfs(adj_flat_list, mass_indices)
    d_curved = multisource_bfs(adj_curved_list, mass_indices)

    # Build sparse adjacency for Markov operators
    A_flat = load_edges_as_sparse_adj(args.edges_flat, N=N, symmetrize=args.symmetrize_P)
    A_curved = load_edges_as_sparse_adj(args.edges_curved, N=N, symmetrize=args.symmetrize_P)

    # Build sparse Markov P (GPU-first)
    P_flat = build_P_from_sparse_adj(A_flat, device=device)
    P_curved = build_P_from_sparse_adj(A_curved, device=device)
    ...
    for k, band in enumerate(shell_bands):
        ...
        # Mean hitting times to the INTERSECT shell (matrix-free)
        h_flat = mean_hitting_time_to_set_iter(
            P_flat, shell_intersect_mask, device=device,
            tol=float(args.ht_tol),
            max_iter=int(args.ht_maxiter),
            check_every=int(args.ht_check_every),
        )
        h_curved = mean_hitting_time_to_set_iter(
            P_curved, shell_intersect_mask, device=device,
            tol=float(args.ht_tol),
            max_iter=int(args.ht_maxiter),
            check_every=int(args.ht_check_every),
        )
    ...
    def stats(vec: np.ndarray, mask: Optional[np.ndarray] = None) -> Dict:
        v = vec[mask] if mask is not None else vec
        v = v[np.isfinite(v)]
        if v.size == 0:
            return {
    ...
    # Correlation G00_med vs rho (T00)
    corr_G00_rho = None
    ...
    report = {
        ...
        "device": str(device),
        "symmetrize_P": bool(args.symmetrize_P),

        "shell_bands": shell_bands,
        "shells": shells_report,

        "ht_tol": float(args.ht_tol),
        "ht_maxiter": int(args.ht_maxiter),
        "ht_check_every": int(args.ht_check_every),
        ...
        "notes": (
            "PP_markov_tau_geometry_v4_multiShellIntersect: STRICT PP Markov τ-geometry. "
            "T00(i) from normalized trace weights (experiences only; combined experiences allowed by input). "
            ...
            "Mean hitting times to S_k computed by matrix-free fixed-point iteration "
            "h=1+P h with boundary h[S]=0. "
